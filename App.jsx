...
const TRIP_START = new Date("2027-04-29"); // יום העצמאות 2027 בערך
const TRIP_WEEKS = 20;

const COUNTRIES = [
  {
    name: "בולגריה", emoji: "🇧🇬", weeks: 1, color: "#E8A87C",
    highlights: ["סופיה", "פלובדיב", "וארנה"],
    mustSee: ["מנסטיר ריל", "ולקו טרנובו", "נסבר העתיקה"],
    tips: "הכי זול בטיול! שווה לאכול בשוק המרכזי בסופיה",
  },
  {
    name: "יוון", emoji: "🇬🇷", weeks: 2, color: "#5B8DB8",
    highlights: ["אתונה", "מטאורה", "כרתים"],
    mustSee: ["האקרופוליס", "מטאורה", "סמוטראקי"],
    tips: "מטאורה זה חובה! נסה לראות שקיעה על מנזרי ההר",
  },
  {
    name: "אלבניה", emoji: "🇦🇱", weeks: 1, color: "#E84855",
    highlights: ["טירנה", "גג'ירוקסטר", "ביתש"],
    mustSee: ["אגם קומאני", "חוף ת'ית'", "קמר"],
    tips: "המדינה הכי מפתיעה בטיול — אנשים מדהימים ומחירים נמוכים",
  },
  {
    name: "מונטנגרו", emoji: "🇲🇪", weeks: 1, color: "#2D6A4F",
    highlights: ["קוטור", "בודבה", "דורמיטור"],
    mustSee: ["ביה קוטורינה", "דורמיטור NP", "אגם שקודר"],
    tips: "קוטור מהיפה שבערי החוף — שווה לעלות לחומה",
  },
  {
    name: "קרואטיה", emoji: "🇭🇷", weeks: 2, color: "#E63946",
    highlights: ["דוברובניק", "ספליט", "פליטביצה"],
    mustSee: ["פליטביצה", "חצי האי פלג'שץ", "הוואר"],
    tips: "פליטביצה = חובה מוחלט. הגע מוקדם בבוקר",
  },
  {
    name: "סלובניה", emoji: "🇸🇮", weeks: 1, color: "#4CAF50",
    highlights: ["לובליאנה", "בלד", "טריגלב"],
    mustSee: ["אגם בלד", "סוקה ואלי", "פוסטויינה"],
    tips: "הכי קומפקטית! ניתן לראות הכל באוטו שכור",
  },
  {
    name: "אוסטריה", emoji: "🇦🇹", weeks: 2, color: "#C41E3A",
    highlights: ["וינה", "זלצבורג", "הלשטאט"],
    mustSee: ["הלשטאט", "גרוסגלוקנר", "וינה מוזיאונים"],
    tips: "גרוסגלוקנר = כביש ההר הכי יפה באירופה",
  },
  {
    name: "איטליה", emoji: "🇮🇹", weeks: 2, color: "#009246",
    highlights: ["דולומיטים", "צינקווה טרה", "רומא"],
    mustSee: ["דולומיטים", "צינקווה טרה", "לאגו די קומו"],
    tips: "התמקד בצפון! הדולומיטים > כל הערים. רומא אפשר בסוף",
  },
  {
    name: "שוויץ", emoji: "🇨🇭", weeks: 3, color: "#FF0000",
    highlights: ["אינטרלאקן", "גרינדלוואלד", "מאטרהורן"],
    mustSee: ["יונגפראו", "לאוטרברונן", "מאטרהורן"],
    tips: "Swiss Pass שווה כאן. הוסטלים SB הכי זולים",
  },
  {
    name: "נורווגיה", emoji: "🇳🇴", weeks: 3, color: "#003087",
    highlights: ["ברגן", "פיורדים", "פרקקסטולן"],
    mustSee: ["טרולטונגה", "פרקקסטולן", "גיירנגרפיורד"],
    tips: "הכי יקר! קנה אוכל בסופר. טרולטונגה = יום שלם",
  },
  {
    name: "שוודיה", emoji: "🇸🇪", weeks: 1, color: "#006AA7",
    highlights: ["סטוקהולם", "גוטנבורג"],
    mustSee: ["ארכיפלג סטוקהולם", "ABBA Museum", "ורמלנד"],
    tips: "סטוקהולם יקרה — בשל בסופר ובישל בהוסטל",
  },
  {
    name: "דנמרק", emoji: "🇩🇰", weeks: 1, color: "#C60C30",
    highlights: ["קופנהגן", "ארהוס"],
    mustSee: ["נוהאבן", "כריסטיאניה", "לואיזיאנה מוזיאון"],
    tips: "קופנהגן = עיר האופניים. שכור אופניים ליום שלם",
  },
];
...
const STORAGE_KEY = "trip_planner_2027";
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ flex: 1, height: 6, background: "rgba(255,255,255,0.08)", borderRadius: 3, overflow: "hidden" }}>
        <div style={{
          ...
          background: pct > 100 ? "#ef4444" : pct > 85 ? "#f59e0b" : "#10b981",
...
export default function TripPlanner() {
  ...
  const persist = useCallback(() => {
    const snapshot = {
      ...
      checked, notes, wishlist,
      savedAt: new Date().toISOString(),
    };
    saveData(snapshot);
    setSavedPulse(true);
    setTimeout(() => setSavedPulse(false), 1500);
  }, [countries, checked, notes, wishlist]);
  ...
  const tabs = [
    { id: "overview", label: "מסלול", icon: "🗺️" },
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          position: "absolute", bottom: -20, left: 60, width: 160, height: 160,
          background: "radial-gradient(circle, rgba(16,185,129,0.1) 0%, transparent 70%)",
          pointerEvents: "none",
        }} />

        <div style={{ maxWidth: 700, margin: "0 auto", position: "relative" }}>
          <div style={{ fontSize: 11, letterSpacing: 3, color: "#818cf8", marginBottom: 6, textTransform: "uppercase" }}>
            אחרי הצבא • קיץ 2027
          </div>
          <h1 style={{ margin: 0, fontSize: "clamp(2rem, 6vw, 3rem)", fontWeight: 900, lineHeight: 1.1 }}>
            <span style={{ color: "#c7d2fe" }}>הטיול</span>{" "}
            <span style={{ color: "#f0ede8" }}>הגדול 🌍</span>
          </h1>
          <p style={{ margin: "6px 0 0", color: "#94a3b8", fontSize: 14 }}>
            12 מדינות • 20 שבועות • שני חברים
          </p>

          <div style={{ display: "flex", gap: 10, marginTop: 16, flexWrap: "wrap", alignItems: "center" }}>
            {[
              { label: "מדינות", value: "12", col: "#818cf8" },
              { label: "שבועות מתוכננים", value: totalWeeks, col: totalWeeks === TRIP_WEEKS ? "#10b981" : totalWeeks > TRIP_WEEKS ? "#ef4444" : "#f59e0b" },
            ...
            <div style={{
              marginRight: "auto",
              fontSize: 12, color: savedPulse ? "#10b981" : "#334155",
              transition: "color 0.4s",
              display: "flex", alignItems: "center", gap: 4,
            }}>
              <span>{savedPulse ? "✓" : "○"}</span>
              <span>{savedPulse ? "נשמר" : "אוטו-שמירה"}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        {tabs.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} style={{
            flex: 1, minWidth: 70, padding: "13px 8px",
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "1.5rem 1rem 3rem" }}>
        ...
        {activeTab === "overview" && (
          <div>
            <p style={{ color: "#64748b", fontSize: 13, marginBottom: 16 }}>
              לחץ על מדינה להוספת הערות, רשימת חלומות, וטיפים 👇
            </p>
            {countries.map((c, i) => (
              <div key={c.name}>
                <div
                  ...
                  style={{
                    ...cardStyle,
                    cursor: "pointer",
                    border: `1px solid ${activeCountry === c.name ? "rgba(129,140,248,0.5)" : "rgba(255,255,255,0.07)"}`,
                    background: activeCountry === c.name ? "rgba(129,140,248,0.08)" : "rgba(255,255,255,0.03)",
                    transition: "all 0.2s",
                    marginBottom: activeCountry === c.name ? 0 : 10,
                    borderRadius: activeCountry === c.name ? "14px 14px 0 0" : 14,
                  }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        ...
                        <span style={{ fontSize: 11, color: "#475569" }}>
                          {fmt(dates[i].start)} – {fmt(dates[i].end)}
                        </span>
                        {(wishlist[c.name]?.length || notes[c.name]) &&
                          <span style={{ color: "#f59e0b", fontSize: 11 }}>📌</span>}
                      </div>
                      <div style={{ marginTop: 3, color: "#64748b", fontSize: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {c.highlights.join(" · ")}
                      </div>
                    </div>

                    <span style={{ color: "#475569", fontSize: 11 }}>{activeCountry === c.name ? "▲" : "▼"}</span>
                  </div>
                </div>
                ...
                {activeCountry === c.name && (
                  <div style={{
                    ...
                    <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
                      <div style={{ flex: 1, minWidth: 160, background: "rgba(0,0,0,0.2)", borderRadius: 10, padding: 12 }}>
                        <div style={{ fontSize: 11, color: "#818cf8", marginBottom: 6, fontWeight: 600 }}>חובה לראות</div>
                        {c.mustSee.map((m, idx) => (
                          <div key={idx} style={{ fontSize: 13, color: "#c7d2fe", marginBottom: 2 }}>• {m}</div>
                        ))}
                      </div>
                      <div style={{ flex: 1, minWidth: 160, background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 10, padding: 12 }}>
                        <div style={{ fontSize: 11, color: "#f59e0b", marginBottom: 6, fontWeight: 600 }}>💡 טיפ</div>
                        <div style={{ fontSize: 12, color: "#d97706", lineHeight: 1.5 }}>{c.tips}</div>
                      </div>
                    </div>

                    {/* personal notes */}
                    <div style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 6 }}>הערות אישיות:</div>
                      <textarea
                        value={notes[c.name] || ""}
                        onChange={e => setNotes(p => ({ ...p, [c.name]: e.target.value }))}
                    ...
                    {/* wishlist */}
                    <div>
                      <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 6 }}>רשימת חלומות ב{c.name}:</div>
                      <div style={{ display: "flex", gap: 8 }}>
                        <input
                          value={newWish[c.name] || ""}
                          onChange={e => setNewWish(p => ({ ...p, [c.name]: e.target.value }))}
                          onKeyDown={e => e.key === "Enter" && addWish(c.name)}
                          placeholder="פעילות / מקום..."
                          style={{
                        ...
                        />
                        <button onClick={() => addWish(c.name)} style={{
                      ...
                      {(wishlist[c.name] || []).map((w, idx) => (
                        <div key={idx} style={{
                          ...
                          <button onClick={() => removeWish(c.name, idx)} style={{
        ...
        {activeTab === "weeks" && (
          <div>
            <div style={{
              background: totalWeeks === TRIP_WEEKS
              ...
              <div style={{ fontSize: 40, fontWeight: 900, color: totalWeeks === TRIP_WEEKS ? "#10b981" : totalWeeks > TRIP_WEEKS ? "#ef4444" : "#f59e0b" }}>
                {totalWeeks}
              </div>
              <div>
                ...
                <div style={{ color: "#94a3b8", fontSize: 13, marginTop: 2 }}>
                  {fmt(TRIP_START)} → {fmt(dates[dates.length - 1].end)}
                </div>
                <div style={{ marginTop: 8 }}>
                  <WeekBar used={totalWeeks} total={TRIP_WEEKS} />
                </div>
              </div>
            </div>

            <p style={{ color: "#64748b", fontSize: 13, marginBottom: 14 }}>
              שנה את מספר השבועות לכל מדינה — הכל מתעדכן אוטומטית 👇
            </p>
              ...
              <div key={c.name} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#475569" }}>
                    {c.weeks * 7} ימים · €{Math.round(DAILY_BUDGET[c.name] * c.weeks * 7)}
                  </div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 0, flexShrink: 0 }}>
                  <button onClick={() => setWeeks(c.name, -0.5)} style={{
                  ...
                  <div style={{
                    ...
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontWeight: 700, fontSize: 15, color: "#c7d2fe",
                  }}>{c.weeks}</div>
                  <button onClick={() => setWeeks(c.name, 0.5)} style={{
            ...
            <div style={{ marginTop: 24 }}>
              ...
              <div style={{ display: "flex", borderRadius: 8, overflow: "hidden", height: 28 }}>
                {countries.map(c => (
                  <div key={c.name} title={`${c.name}: ${c.weeks} שב'`} style={{
                    flex: c.weeks,
              ...
              <div style={{ display: "flex", marginTop: 4 }}>
                {countries.map(c => (
                  <div key={c.name} style={{
                    flex: c.weeks, fontSize: 9, color: "#475569",
            ...
            <div style={{
              ...
              <div style={{ position: "relative", width: 64, height: 64, flexShrink: 0 }}>
                <svg viewBox="0 0 64 64" style={{ transform: "rotate(-90deg)" }}>
                  ...
                  <circle cx="32" cy="32" r="27" fill="none" stroke="#818cf8" strokeWidth="6"
                    strokeDasharray={`${progress * 1.7} 170`} strokeLinecap="round" />
                </svg>
                <div style={{
              ...
              <div>
                ...
                <div style={{ color: "#94a3b8", fontSize: 13 }}>
                  {totalChecked} מתוך {totalItems} פריטים הושלמו
                </div>
              </div>
            </div>
                ...
                <div key={cat} style={{ marginBottom: 10 }}>
                  <button onClick={() => setExpandedCat(isOpen ? null : cat)} style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{
                        background: catDone === items.length ? "rgba(16,185,129,0.2)" : "rgba(255,255,255,0.06)",
                        color: catDone === items.length ? "#10b981" : "#94a3b8",
                      ...
                      {items.map(item => (
                        <div key={item.id} onClick={() => setChecked(p => ({ ...p, [item.id]: !p[item.id] }))} style={{
                          ...
                          <div style={{
                            ...
                            border: `2px solid ${checked[item.id] ? "#818cf8" : "rgba(255,255,255,0.2)"}`,
                            background: checked[item.id] ? "#818cf8" : "transparent",
                            display: "flex", alignItems: "center", justifyContent: "center",
                            transition: "all 0.15s",
                          }}>
                            {checked[item.id] && <span style={{ color: "white", fontSize: 12 }}>✓</span>}
                          </div>
                          <span style={{ fontSize: 14, textDecoration: checked[item.id] ? "line-through" : "none" }}>
                            {item.text}
                          </span>
                        </div>
                      ))}
        ...
        {activeTab === "budget" && (
          <div>
            <div style={{
              ...
              <div style={{ fontSize: 46, fontWeight: 900, color: "#10b981" }}>
                €{Math.round(totalBudget).toLocaleString()}
              </div>
              <div style={{ color: "#64748b", fontSize: 13, marginTop: 4 }}>
                ≈ ₪{Math.round(totalBudget * 3.8).toLocaleString()} · לאדם אחד
              </div>
              <div style={{ fontSize: 12, color: "#475569", marginTop: 8 }}>
                * הוסטלים + תחבורה + אוכל. לא כולל טיסות ו-Eurail Pass
              </div>
            </div>
            ...
            <div style={{
              ...
              {[
                ...
                <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, fontSize: 13 }}>
                  ...
                  <div>
                    <span style={{ color: "#fbbf24", fontWeight: 600 }}>{title}</span>
